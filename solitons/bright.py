...
def bright_soliton(x, t, A, w_number, x0, theta0):
    ...
    real_part = A / np.cosh(A * (x - 2 * w_number * t - x0)) * np.cos(w_number * x - (w_number**2 - A**2) * t + theta0)
    imag_part = A / np.cosh(A * (x - 2 * w_number * t - x0)) * np.sin(w_number * x - (w_number**2 - A**2) * t + theta0)
